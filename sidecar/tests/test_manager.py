...
class TestCircuitBreaker:
    ...
    def test_initial_state_is_closed(self):
        assert self.breaker.get_state() == _CircuitState.CLOSED
    ...
    def test_three_failures_opens_breaker(self):
        for _ in range(3):
            self.breaker.record_failure()
        assert self.breaker.get_state() == _CircuitState.OPEN
    ...
    def test_open_to_half_open_after_timeout(self):
        ...
        assert self.breaker.get_state() == _CircuitState.HALF_OPEN
        assert self.breaker.allow_request() is True
    ...
    def test_custom_threshold(self):
        ...
        assert breaker.get_state() == _CircuitState.OPEN
        assert breaker.allow_request() is False

    def test_get_state_is_method_not_property(self):
        """get_state() is a method (not a property) to avoid side effects."""
        assert callable(self.breaker.get_state)
        assert self.breaker.get_state() == _CircuitState.CLOSED
...
class TestManagerExplicitRouting:
    ...
    async def test_explicit_backend_calls_correct_engine(self):
        result = await self.manager.recognize(b"img", backend="mathpix")
        assert result.backend == "mathpix"
        self.mathpix.recognize.assert_called_once()

    async def test_explicit_backend_does_not_call_other_engines(self):
        await self.manager.recognize(b"img", backend="pix2text")
    ...
    async def test_unknown_backend_raises(self):
        with pytest.raises(OcrError, match="Unknown OCR backend"):
            await self.manager.recognize(b"img", backend="nonexistent")

    async def test_explicit_backend_respects_circuit_breaker(self):
    ...
    async def test_options_passed_through(self):
        ...
        await self.manager.recognize(b"img", backend="pix2text", options=opts)
...
class TestManagerCostRouting:
    async def test_auto_picks_cheapest_engine(self):
...
class TestManagerCircuitBreaker:
    async def test_consecutive_failures_trip_breaker(self):
        ...
        for _ in range(2):
            with pytest.raises(NetworkError):
                await manager.recognize(b"img", backend="openai")
        assert manager.get_breaker("openai").get_state() == _CircuitState.CLOSED

        # 3rd failure: breaker opens
        with pytest.raises(NetworkError):
            await manager.recognize(b"img", backend="openai")
        assert manager.get_breaker("openai").get_state() == _CircuitState.OPEN

    async def test_breaker_blocks_after_open(self):
        engine = _make_engine("openai", fail=True)
    ...
    async def test_success_resets_breaker_counter(self):
    ...
    async def test_available_engines_excludes_open(self):
...
class TestManagerFallback:
    ...
    async def test_fallback_chain_llm_to_mathpix_to_pix2text(self):
    ...
    async def test_fallback_skips_circuit_broken_engines(self):
        ...
        for _ in range(3):
            with pytest.raises(NetworkError):
                await manager.recognize(b"img", backend="pix2text")
    ...
    async def test_all_engines_fail_raises_last_error(self):
        ...
        with pytest.raises(NetworkError):
            await manager.recognize(b"img")

    async def test_auto_mode_does_not_raise_for_normal_operation(self):
    ...
    async def test_fallback_preserves_result_from_successful_engine(self):
...
class TestManagerEdgeCases:
    async def test_single_engine_manager(self):
    ...
    async def test_default_options_used_when_none(self):
    ...
    async def test_recognize_returns_ocr_result_not_coroutine(self):
        """recognize() must return OcrResult, not a coroutine object.

        Regression test: previously recognize() was sync but called async
        engine methods, returning a coroutine instead of the actual result.
        """
        engine = _make_engine("pix2text")
        manager = EngineManager(engines={"pix2text": engine})
        result = await manager.recognize(b"img")
    ...
    async def test_recognize_auto_returns_ocr_result_not_coroutine(self):
        """Auto-mode recognize() must also return OcrResult, not a coroutine."""
        engine = _make_engine("pix2text")
        manager = EngineManager(engines={"pix2text": engine})
        result = await manager.recognize(b"img", backend="auto")
        assert isinstance(result, OcrResult)
        assert not inspect.iscoroutine(result)
